...
struct SwipeCardView: View {
        ...
        NavigationStack {
            GeometryReader { geometry in
                VStack(spacing: 20) {
                    ...
                    HStack(spacing: 40) {
                        CircleButton(icon: "arrow.uturn.backward", tint: .blue) {
                            undoLastSwipe()
                        }
    ...
    private var cardStack: some View {
        ForEach((0..<min(2, group.assets.count - currentIndex)).reversed(), id: \.self) { index in
            ...
            if actualIndex < preloadedImages.count, let image = preloadedImages[actualIndex] {
                ZStack {
                    ...
                    if index == 0, let swipeLabel = swipeLabel {
                        Text(swipeLabel.uppercased())
                            ...
                            .offset(x: cardOffset.width + dragOffset.width, y: -UIScreen.main.bounds.height / 4)
                    }
                }
                ...
                spinnerCard(width: UIScreen.main.bounds.width * 0.85, index: index)
            }
        }
    }
    ...
    private func undoLastSwipe() {
        ...
        photoManager.unmarkForFavourite(asset)
        photoManager.removeAsset(asset, fromAlbumNamed: "Saved")
        toast.show("Undo successful")
    }
